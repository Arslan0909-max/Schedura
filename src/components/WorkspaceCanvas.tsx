...
export const WorkspaceCanvas: React.FC<WorkspaceCanvasProps> = ({
  ...
  return (
    ...
    >
      ...
      <header className="h-14 border-b border-zinc-200/60 dark:border-white/10 px-6 flex items-center justify-between apple-liquid-glass-subtle backdrop-blur-2xl shrink-0 z-10">
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="relative">
            <button
              ...
              className={`px-3.5 py-1.5 rounded-xl border border-zinc-200/80 dark:border-white/10 bg-white dark:bg-[#181920] text-[13px] font-medium apple-stretcher apple-puncher flex items-center gap-1.5 shadow-xs cursor-pointer ${
                activeTimetable
                  ? 'text-zinc-700 dark:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-600'
            ...
            {showExportMenu && activeTimetable && (
              <div className="absolute top-10 right-0 w-64 p-2 apple-liquid-glass-menu rounded-2xl shadow-2xl z-50 text-[12.5px] animate-apple-menu space-y-0.5">
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-xl text-zinc-800 dark:text-zinc-200 hover:bg-zinc-100 dark:hover:bg-zinc-800 apple-stretcher apple-puncher transition-all flex items-center gap-2.5"
                >
          ...
          <button
            ...
            onClick={handleShare}
            disabled={!activeTimetable}
            className={`px-3.5 py-1.5 rounded-xl text-[13px] font-medium apple-stretcher apple-puncher flex items-center gap-2 shadow-sm cursor-pointer ${
              activeTimetable
                ? 'bg-zinc-900 dark:bg-indigo-600 hover:bg-zinc-800 dark:hover:bg-indigo-700 text-white'
      ...
      {activeTimetable && (
        <div className="px-6 py-2.5 border-b border-zinc-200/60 dark:border-white/10 apple-liquid-glass-subtle backdrop-blur-2xl flex flex-wrap items-center justify-between gap-3 shrink-0">
          ...
          <div className="flex items-center gap-3 overflow-x-auto py-0.5">
            <div className="flex items-center gap-1.5">
              {allTimetables.map((t) => (
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-xl text-[12px] font-medium apple-stretcher apple-puncher whitespace-nowrap ${
                    t.id === activeTimetable.id
                      ? 'bg-zinc-900 dark:bg-indigo-600 text-white shadow-xs'
                      : 'bg-white/80 dark:bg-[#181920]/80 border border-zinc-200/70 dark:border-white/10 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-800'
                  }`}
                >
            ...
            {/* View Mode Switcher */}
            <div className="flex items-center bg-zinc-200/70 dark:bg-zinc-800/80 p-0.5 rounded-xl border border-zinc-200/80 dark:border-white/10">
              <button
                type="button"
                onClick={() => setViewMode('grid')}
                className={`px-2.5 py-1 rounded-lg text-[11.5px] font-medium apple-stretcher apple-puncher transition-all ${
                  viewMode === 'grid'
      ...
      <div className="flex-1 overflow-auto p-4 md:p-6 relative">
        ...
        {isVoiceProcessing ? (
          ...
          >
            <div className="w-16 h-16 rounded-2xl apple-liquid-glass-card border border-white/60 dark:border-white/10 shadow-lg flex items-center justify-center text-zinc-500 dark:text-zinc-400 mb-4 backdrop-blur-xl">
              <Calendar className="w-8 h-8 stroke-[1.5]" />
            </div>
            <h2 className="text-[18px] font-semibold text-zinc-900 dark:text-white tracking-tight mb-1.5">
          ...
          <div id="room-allocation-matrix" className="space-y-4 animate-in fade-in duration-200">
            <div className="flex items-center justify-between">
              ...
              <div className="text-[12px] text-zinc-600 dark:text-zinc-300 apple-liquid-glass-subtle px-3.5 py-1.5 rounded-xl border border-zinc-200/80 dark:border-white/10 shadow-xs">
                {roomsData.length} Rooms Allocated
              </div>
            </div>

            {roomsData.length === 0 ? (
              <div className="apple-liquid-glass-card rounded-3xl border border-zinc-200/80 dark:border-white/10 p-8 text-center text-zinc-400 dark:text-zinc-500">
                No classrooms assigned yet.
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {roomsData.map(({ room, slots }) => (
                  <div key={room} className="apple-liquid-glass-card rounded-3xl border border-zinc-200/80 dark:border-white/10 p-4 shadow-sm">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-white/5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-zinc-100 dark:bg-zinc-800 border border-zinc-200/60 dark:border-white/10 flex items-center justify-center text-zinc-800 dark:text-zinc-200 font-bold text-[12px]">
                          {room.slice(0, 3)}
                        </div>
                        <div>
                    ...
                    <div className="mt-3 space-y-2">
                      {slots.map((s) => (
                        <div
                          ...
                          className="p-2.5 rounded-2xl border border-zinc-100 dark:border-white/5 bg-zinc-50/60 dark:bg-white/[0.03] hover:bg-zinc-100 dark:hover:bg-white/[0.08] apple-stretcher apple-puncher cursor-pointer transition-all text-[12px] flex items-center justify-between"
                        >
                          <div>
            ...
            {teachersData.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {teachersData.map(({ teacher, slots, totalHours }) => (
                  <div key={teacher} className="apple-liquid-glass-card rounded-3xl border border-zinc-200/80 dark:border-white/10 p-4 shadow-sm">
                    <div className="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-white/5">
                      <div className="flex items-center gap-2.5">
          ...
          <div
            ...
            className="w-full apple-liquid-glass-card rounded-[28px] border border-zinc-200/80 dark:border-white/10 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200"
          >
            {/* Timetable Title Header inside Card */}
            <div className="px-6 py-4 border-b border-zinc-100 dark:border-white/5 flex items-center justify-between bg-zinc-50/50 dark:bg-white/[0.02]">
              <div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse text-left min-w-[760px]">
                <thead>
                  <tr className="border-b border-zinc-200/80 dark:border-white/10 bg-zinc-50/80 dark:bg-white/[0.02]">
                    <th className="w-24 px-4 py-3.5 text-[11px] font-semibold text-zinc-400 dark:text-zinc-500 uppercase tracking-wider sticky left-0 bg-zinc-50/95 dark:bg-[#15161c] z-10 border-r border-zinc-200/60 dark:border-white/10">
                      Day
                    </th>
                    {activeTimetable.timeSlots.map((slot) => (
                      <th
                        key={slot}
                        className="px-3 py-3 text-center text-[11.5px] font-semibold text-zinc-600 dark:text-zinc-300 border-r border-zinc-200/60 dark:border-white/10 last:border-r-0"
                      >
                ...
                <tbody className="divide-y divide-zinc-200/60 dark:divide-white/10">
                  {activeTimetable.days.map((day) => (
                    <tr key={day} className="group hover:bg-zinc-50/40 dark:hover:bg-white/[0.02] transition-colors">
                      {/* Day Header Cell */}
                      <td className="px-4 py-3 text-[13px] font-semibold text-zinc-900 dark:text-zinc-100 sticky left-0 bg-white/95 dark:bg-[#15161c] group-hover:bg-zinc-50/90 dark:group-hover:bg-[#181920] z-10 border-r border-zinc-200/60 dark:border-white/10 shadow-2xs">
                      ...
                      {activeTimetable.timeSlots.map((timeSlot) => {
                        ...
                        return (
                          <td
                            ...
                            className={`p-2 border-r border-zinc-200/60 dark:border-white/10 last:border-r-0 h-24 align-top transition-all duration-150 relative ${
                              isTargetCell
                                ? dragWarning
                            ...
                            {cellSlot ? (
                              cellSlot.isBreak ? (
                                ...
                                <div
                                  ...
                                  className="w-full h-full rounded-2xl p-2.5 bg-zinc-100/80 dark:bg-white/[0.04] border border-zinc-200/60 dark:border-white/10 flex flex-col justify-center items-center text-center cursor-pointer hover:bg-zinc-200/60 dark:hover:bg-white/[0.08] apple-stretcher apple-puncher transition-all group/break"
                                >
                                ...
                                <div
                                  ...
                                  onClick={() => setEditingSlot(cellSlot)}
                                  className={`w-full h-full rounded-2xl p-2.5 border shadow-2xs cursor-grab active:cursor-grabbing apple-stretcher apple-puncher transition-all flex flex-col justify-between relative group/card ${getColorClasses(
                                    cellSlot.color
