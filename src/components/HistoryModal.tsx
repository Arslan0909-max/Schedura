...
export const HistoryModal: React.FC<HistoryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="w-full max-w-lg bg-white/95 backdrop-blur-2xl border border-zinc-200/80 rounded-3xl p-6 shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between pb-4 border-b border-zinc-100 mb-4">
          <div>
            <h3 className="text-[17px] font-semibold text-zinc-900">Schedule History</h3>
        ...
        {timetables.length === 0 ? (
          ...
          <div className="space-y-2.5 max-h-[50vh] overflow-y-auto pr-1">
            {timetables.map((t) => (
              <div
                key={t.id}
                className="p-3.5 rounded-2xl border border-zinc-200/80 bg-white hover:bg-zinc-50 flex items-center justify-between transition-colors"
              >
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl bg-black hover:bg-zinc-800 text-white text-[12px] font-medium flex items-center gap-1"
                  >
                    <span>Open</span>
        ...
        <div className="pt-4 mt-4 border-t border-zinc-100 flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-xl border border-zinc-200 hover:bg-zinc-50 text-zinc-700 text-[13px] font-medium"
          >
            Close
          </button>
        </div>
